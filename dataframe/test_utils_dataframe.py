...
@pytest.mark.parametrize(
    ...
    [
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [4, 5, 6]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [4, 5, 6]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
...
@pytest.mark.parametrize(
    ...
    [
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 4]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
        ...
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [1, 2, 3]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [3, 2, 1]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [1, 2, 3]}), npartitions=1),
...
def test_assert_eq_false(a, b):
    with pytest.raises(AssertionError, match="DataFrame.iloc.*column name.*values are different"):
        assert_eq(a, b)
...
@pytest.mark.parametrize(
    ...
    [
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3, 4]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": []}), npartitions=1),
...
@pytest.mark.parametrize(
    ...
    [
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"y": [1, 2, 3]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [5, 6, 7]}), npartitions=1),
            dd.from_pandas(pd.DataFrame({"y": [5, 6, 7], "x": [1, 2, 3]}), npartitions=1)
        ),
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3]}), npartitions=1),
        ...
        (
            dd.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [5, 6, 7]}), npartitions=1),
            pd.DataFrame({"y": [5, 6, 7], "x": [1, 2, 3]})
        ),
    ],
)
def test_assert_eq_checks_columns(a, b):
    with pytest.raises(AssertionError, match="DataFrame.columns values are different"):
        assert_eq(a, b)
